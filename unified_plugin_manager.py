        ...
        try:
            ...
            if not backend_integration.is_authenticated():
                self.error_occurred.emit(
                    "Usuario debe estar autenticado para descargar plugins"
                )
                return
            if not backend_integration.has_plugin_access(self.plugin_id):
                self.error_occurred.emit(
                    "No tienes acceso a este plugin. Por favor, compra la suscripción primero."
                )
                return
            ...
            if response and response.success:
                ...
                self.error_occurred.emit(
                    msg or "Error descargando plugin desde el backend"
                )
...
class UnifiedPluginManager(QObject):
    ...
    def __init__(self, auth_manager: AuthManager = None, parent=None):
        ...
        self.validation_interval = (
            self.config.get_license_validation_interval()
        )  # 300s = 5 min
        self.max_validation_attempts = 3
        self.validation_attempts = {}
        logger.info(
            f"Plugin Manager initialized - Validation interval: {self.validation_interval}s"
        )
        ...
        if self.auth_manager:
            ...
            self.auth_manager.plugin_license_changed.connect(
                self.on_plugin_license_changed
            )
    ...
    def load_configuration(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                print(
                    f"[UnifiedPluginManager] Configuración cargada desde {self.config_file}"
                )
        except Exception as e:
    ...
    def save_configuration(self):
        ...
        try:
            ...
            print(
                f"[UnifiedPluginManager] Configuración guardada en {self.config_file}"
            )
        except Exception as e:
    ...
    def load_available_plugins(self) -> List[PluginInfo]:
        ...
        try:
            if (
                not self.auth_manager
                or not self.auth_manager.auth_state.is_authenticated
            ):
                return []
            # Usar integración con backend
            from backend_integration import backend_integration
    ...
    def get_plugin_access(self, plugin_id: str) -> PluginAccessInfo:
        ...
        if plugin_info_file.exists():
            try:
                with open(plugin_info_file, "r", encoding="utf-8") as f:
                    ...
                    is_free_plugin = not plugin_info_data.get(
                        "premium", True
                    )  # Default to premium if not specified
                    logger.info(
                        f"[DEBUG] Plugin {plugin_id}: premium={plugin_info_data.get('premium', True)}, is_free_plugin={is_free_plugin}"
                    )
            except Exception as e:
        ...
        if is_free_plugin:
            logger.info(
                f"Plugin {plugin_id} access granted: Free plugin (no auth required)"
            )
            return PluginAccessInfo(
                plugin_id=plugin_id,
                plugin_name=(
                    plugin_info_data.get("name", "Plugin Desconocido")
                    if "plugin_info_data" in locals()
                    else "Plugin Desconocido"
                ),
        ...
        logger.info(
            f"[DEBUG] Plugin {plugin_id}: is_licensed={is_licensed}, license_info={'EXISTS' if license_info else 'NONE'}"
        )
        ...
        if self._is_trial_active(plugin_id):
            ...
            logger.info(
                f"Plugin {plugin_id} access granted: Trial ({trial_remaining} days remaining)"
            )
            return PluginAccessInfo(
                plugin_id=plugin_id,
                plugin_name=config["name"],
    ...
    def request_feature_access(
        self, plugin_id: str, feature: str, parent_widget: QWidget = None
    ) -> bool:
        ...
        self.access_denied.emit(
            plugin_id, f"La característica '{feature}' requiere acceso premium"
        )
        return False
    ...
    def _show_upgrade_dialog(
        ...
        can_trial = not self._is_trial_active(
            plugin_id
        ) and not self.auth_manager.is_plugin_licensed(plugin_id)
        msg = QMessageBox(parent_widget)
        msg.setWindowTitle("Característica Premium")
        msg.setIcon(QMessageBox.Information)
        if can_trial:
            msg.setText(
                f"La característica '{feature}' es una característica premium de {plugin_name}."
            )
            ...
            msg.setText(
                f"La característica '{feature}' requiere una suscripción premium de {plugin_name}."
            )
            msg.setInformativeText(
                "Visita nuestro sitio web para obtener acceso premium."
            )
            upgrade_btn = msg.addButton("Comprar Premium", QMessageBox.ActionRole)
            cancel_btn = msg.addButton("Cancelar", QMessageBox.RejectRole)
    ...
    def _on_download_completed(self, plugin_id: str, success: bool):
        ...
        if success:
            ...
            if self.load_plugin(plugin_id):
                ...
                print(
                    f"[UnifiedPluginManager] Plugin '{plugin_id}' instalado y cargado exitosamente"
                )
            else:
                self.plugin_installed.emit(
                    plugin_id
                )  # Emitir señal aunque no se cargue
                print(
                    f"[UnifiedPluginManager] Plugin '{plugin_id}' instalado pero no se pudo cargar dinámicamente"
                )
        else:
    ...
    def uninstall_plugin(self, plugin_id: str) -> bool:
        ...
        try:
            ...
            if plugin_dir.exists():
                ...
                print(
                    f"[UnifiedPluginManager] Plugin '{plugin_id}' desinstalado exitosamente"
                )
                return True
            return False
        except Exception as e:
            print(
                f"[UnifiedPluginManager] Error desinstalando plugin '{plugin_id}': {e}"
            )
            return False
    ...
    def _validate_plugin_access(self, plugin_id: str):
        ...
        if not self.auth_manager.auth_state.is_authenticated:
            logger.warning(
                f"Plugin {plugin_id} validation failed: User not authenticated"
            )
    ...
    def force_validation(self, plugin_id: str = None):
        ...
        logger.info(
            f"Forced validation for {'all plugins' if not plugin_id else plugin_id}"
        )
    ...
    def get_validation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "auth_connected": (
                self.auth_manager.is_connected if self.auth_manager else False
            ),
            ...
            if plugin_id in self.plugins:
                ...
                if hasattr(plugin_instance, "shutdown"):
                    try:
                        ...
                        logger.error(
                            f"Error calling shutdown() on plugin {plugin_id}: {e}"
                        )
    ...
    def load_all_installed_plugins(self) -> Dict[str, bool]:
        ...
        for plugin_id in installed:
            ...
            if (
                access_info.is_licensed
                or access_info.access_level == PluginAccessLevel.FREE
            ):
                success = self.load_plugin(plugin_id)
                results[plugin_id] = success
                if success:
                    logger.info(
                        f"Loaded plugin: {plugin_id} (access_level={access_info.access_level.value})"
                    )
                else:
                    logger.warning(f"Failed to load plugin: {plugin_id}")
            else:
                logger.info(
                    f"Skipping plugin {plugin_id} - no access (access_level={access_info.access_level.value}, is_licensed={access_info.is_licensed})"
                )
